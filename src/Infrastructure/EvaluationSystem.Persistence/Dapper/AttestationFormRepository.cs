...
namespace EvaluationSystem.Persistence.Dapper
{
    public class AttestationFormRepository : BaseRepository<AttestationForm>, IAttestationFormRepository
    {
        ...
        {
            string query = @"SELECT f.Id AS IdForm, f.[Name] AS NameForm, fm.Position AS ModulePosition, m.Id AS IdModule, m.[Name] AS NameModule, m.Description, mq.Position AS QuestionPosition, 
                                    q.Id AS IdQuestion, q.[Name] AS NameQuestion, q.[Type], a.Id AS IdAnswer, a.IsDefault, a.AnswerText AS AnswerText 
                                            FROM AttestationForm AS f
                                            LEFT JOIN AttestationFormModule AS fm ON f.Id = fm.IdForm
